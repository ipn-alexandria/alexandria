...
public class AdminNavegarF1Servlet extends HttpServlet {
	...
	try (OutputStream out = response.getOutputStream()) {
		...
		response.sendRedirect("jsp/administrador/evaluarvideo.jsp");
	    } else if (idtipom == 0) {
		...
		session.setAttribute("rutaMaterial", contextPath + "/PDFServlet?idMaterial=" + idtip);
                session.setAttribute("idMaterial", idtip);
		response.sendRedirect("jsp/administrador/evaluarpdf.jsp");
	    }
	}
    }

...
public class ProfNavegarF1Servlet extends HttpServlet {
	...
	try (OutputStream out = response.getOutputStream()) {
		...
		response.sendRedirect("jsp/profesor/evaluarvideo.jsp");
	    } else if (idtipom == 0) {
		...
		session.setAttribute("rutaMaterial", contextPath + "/PDFServlet?idMaterial=" + idtip);
                session.setAttribute("idMaterial", idtip);
		response.sendRedirect("jsp/profesor/evaluarpdf.jsp");
	    }
	}
    }

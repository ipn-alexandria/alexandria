 ...
 * @author Alan
 */
public class ProfNavegarF2Servlet extends HttpServlet {
   ...
   protected void processRequest(HttpServletRequest request, HttpServletResponse response)
	...
	try (OutputStream out = response.getOutputStream()) {
		...
		response.sendRedirect("jsp/profesor/evaluarvideo2.jsp");
	    } else if (idtipom == 0) {
		...
		session.setAttribute("rutaMaterial", contextPath + "/PDFServlet?idMaterial=" + idtip);
                session.setAttribute("idMaterial", idtip);
		response.sendRedirect("jsp/profesor/evaluarpdf2.jsp");
	    }
	}
    }
